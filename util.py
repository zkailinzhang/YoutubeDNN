
import pandas as pd
import numpy as np
import re
import itertools
import pickle 
import json 
#对

file = pd.read_csv('/home/tv/Downloads/query_result_desc.csv',)

tags = file['tags']

#cate_tag_dict = file.loc[:,['category','tags']].set_index().T.to_dict()
cate_tag_dict = file.loc[:,['category','tags']].T.to_dict()



#cate_tag_dict_new =  v if v['tags']=='nan'   for  k,v 
for k,v in cate_tag_dict.items():
    if 'nan' in str(v['tags']):
        v['tags']=v['category']
        cate_tag_dict[k]=v
    #tmp = list(re.split('[\：|\~|\;|\，|\、|\t|\s]',v['tags']))
    cate_tag_dict[k]['tags'] = re.split('[\：|\~|\;|\，|\、|\t|\s]',v['tags'])


tags_raw = [value['tags'] for value in cate_tag_dict.values()]

# [\~|\·|\;|\，|\、|\t|\s]
tags_dicc =list( map(lambda x: re.split('[\：|\~|\;|\，|\、|\t|\s]',x), tags_raw))


tags_ll = list(itertools.chain.from_iterable(list(tags_dicc)))
tags_l = list(set(tags_ll))
len(tags_l)


cate_raw = [value['category'] for value in cate_tag_dict.values()]
cate_l = list(set(cate_raw))

length = len(tags_l)+len(cate_l)

index = [x for x in range(length)]

cate_tag_dict = dict(zip(cate_l+tags_l,index))

cate_tag_dict['UNK']=length

output = open("cate_tag_dict.pkl",'wb')
pickle.dump(cate_tag_dict,output)

with open('cate_tag_dict.json','w') as ff:
    json.dump(cate_tag_dict,ff,ensure_ascii=False)

dataframe.apply(lambda row:row['category'],row['tags'] = cat_index[row['category']],cat_index[row['tags']],axis=1)




def get_index(input):
    item_info_dict = defaultdict(dict)
    item_id_dict = defaultdict(dict)
    item_index_dict = dict()
    category_index_dict = dict()
    
    words  = input
    vocabulary_size = 50000
    count = [['UNK',-1]]
    #去重. 
    #count.extend(collections.Counter(words).most_common(vocabulary_size-1))
    count.extend(collections.Counter(words))
    # 法2 传过来是一个列表,
    
    dictionary = dict()
    
    #按出现次数大学排序,
    #count_sort = sorted(dict(count) ,key=lambda x: x[1])

    for word in count:
        dictionary[word] = len(dictionary)
    
    data = list()

    unk_count = 0
    for word in words:
        if word in dictionary:
            index  = dictionary[word]
        else:
            index = 0   # dictionary['UNK']

            unk_count +=1
        data.append(index)

    count[0][1] = unk_count
    reverse_dict = dict(zip(dictionary.values(),dictionary.keys()))
        
    return data,count,reverse_dict


dataframe = get_data_from_csv('/home/tv/Downloads/query_result_notag.csv')
item_id_index,count,reverse_dict = get_index (dataframe['item_id'])
item_id_index,count,reverse_dict = get_index (dataframe['distinct_id'])

# list(itertools.chain.from_iterable(ll1))  
# ll1 =tr.words_all_filters

# tr = TextRank4Keyword()
# tagta.values.tolist() 
# tagta = tags[tags.notnull()]